...
class CommunityFeed extends PureComponent {
  constructor(props) {
    super(props);
  ...
  componentDidMount(){
    ...
    console.log("CommunityFeed this.props.match",this.props.match)

		if(params && params.community){
	    postService.getPostsbyCommunity(params.community).then(res => {
	          console.log('load getPostsbyCommunity');
	          console.log(res);
      	...
      	<div className="columns is-desktop is-gapless is-centered has-marg1in-5 is-ce1ntered has-marg1in-top-10">
      		<div className="column is-half-desktop is-squ1are has-marg1in-10  has-backgro1und-white has-padd1ing-top-0">
      			<div className="is-square has-margi1n-10  has-background-white">
							<div className="is-size-5 has-margin-bottom-5">{`Community /`+params.community}</div>
              {emojifilter}
      				{message}
      				{search}
              {data && data.map(hit =>
