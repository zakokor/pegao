...
class PostsEmoji extends PureComponent {
  constructor(props) {
    super(props);
  ...
  fetchPosts = () => {
    ...
    console.log('PostsEmoji params.username:'+params.username);
    console.log("PostsEmoji this.props.match",this.props.match)
    console.log("PostsEmoji emoji",params.emoji)
    
    //console.time("timer fetchPosts");
    
    if(params && params.username && params.emoji && this.state.next_page){
      postService.getPostsbyUserEmoji(params.username,params.emoji,this.state.next_page).then(res => {
